...
// @ts-ignore
const maybeExpoVersion = (_global$Expo = global.Expo) === null || _global$Expo === void 0 ? void 0 : _global$Expo.Constants.manifest.sdkVersion.split('.')[0];
const isInsufficientExpoVersion = maybeExpoVersion ? Number(maybeExpoVersion) <= 36 : maybeExpoVersion === 'UNVERSIONED';
const EPSILON = 0.01;
...
const MaybeScreenContainer = (_ref) => {
  ...
  if (enabled && screensEnabled()) {
    return React.createElement(ScreenContainer, _extends({
      style: style
    }, rest));
  }
...
export default class CardStack extends React.Component {
  constructor() {
    ...
    _defineProperty(this, "handleHeaderLayout", (_ref3) => {
      let {
        route,
        height
      } = _ref3;
  ...
  render() {
    ...
    const isScreensEnabled = Platform.OS !== 'ios' && (isInsufficientExpoVersion ? mode !== 'modal' : true);
    return React.createElement(React.Fragment, null, React.createElement(MaybeScreenContainer, {
      ...
      const scene = scenes[index]; // Display current screen and a screen beneath.

      let isScreenActive = index >= self.length - 2 ? 1 : 0;

      if (isInsufficientExpoVersion) {
        isScreenActive = index === self.length - 1 ? 1 : Platform.OS === 'android' ? scene.progress.next ? scene.progress.next.interpolate({
