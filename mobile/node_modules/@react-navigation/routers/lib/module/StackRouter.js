  ...
  const router = _objectSpread({}, BaseRouter, {
    ...
    getInitialState(_ref) {
      ...
      return {
        stale: false,
        type: 'stack',
        key: "stack-".concat(shortid()),
        index: 0,
        routeNames,
        routes: [{
          key: "".concat(initialRouteName, "-").concat(shortid()),
    ...
    getRehydratedState(partialState, _ref2) {
      ...
      const routes = state.routes.filter(route => routeNames.includes(route.name)).map(route => _objectSpread({}, route, {
        key: route.key || "".concat(route.name, "-").concat(shortid()),
      ...
      switch (action.type) {
        case 'REPLACE':
          {
            ...
            return _objectSpread({}, state, {
              routes: state.routes.map((route, i) => i === index ? {
                key: key !== undefined ? key : "".concat(name, "-").concat(shortid()),
                name,
        ...
        case 'PUSH':
          if (state.routeNames.includes(action.payload.name)) {
            return _objectSpread({}, state, {
              index: state.index + 1,
              routes: [...state.routes, {
                key: action.payload.key === undefined ? "".concat(action.payload.name, "-").concat(shortid()) : action.payload.key,
        ...
        case 'NAVIGATE':
          if (action.payload.key || action.payload.name && state.routeNames.includes(action.payload.name)) {
            ...
            return _objectSpread({}, state, {
              index,
              routes: [...state.routes.slice(0, index), action.payload.params !== undefined ? _objectSpread({}, state.routes[index], {
                params: _objectSpread({}, state.routes[index].params, {}, action.payload.params)
              }) : state.routes[index]]
            });
          }
