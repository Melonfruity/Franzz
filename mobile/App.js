...
const App = () => {
  ...
  const guest = usernameObj => {
    service
      .guest(usernameObj)
      .then(({ error, guest, userID, username, authorization }) => {
        if (!error) {
          setState(prev => ({
            ...prev,
            guest,
  ...
  const createChannel = (channelName, cb) => {
    ...
    socket.emit("create channel", createChannelObj, channelData => {
      ...
      axios.post(`${serverURL}/api/photos/createEmptyFolder`, {
        ...
        setState(prev => ({
          ...
          channelStates: {
            ...
            [channelID]: {
              ...
              messages: [
                newMessageObj,
                ...prev.channelStates[channelID].messages
              ]
            }
          }
        }));
